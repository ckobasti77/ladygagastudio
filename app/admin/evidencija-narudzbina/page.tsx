...
type PaymentFilter = "all" | "awaiting" | "ips" | "cod";
...
/** IPS porudžbina koja čeka uplatu ne sme da ode kuriru kao pouzeće. */
function isAwaitingPayment(order: AdminOrder) {
  return order.paymentMethod === "ips" && order.paymentStatus !== "confirmed";
}
...
export default function AdminOrdersLedgerPage() {
  ...
  const confirmOrderPayment = useMutation(api.orders.confirmOrderPayment) as unknown as (args: {
    orderId: string;
    confirmed?: boolean;
  ...
  const filteredOrders = useMemo(() => {
    ...
    const list = orders.filter((order) => {
      ...
      if (paymentFilter === "awaiting" && !isAwaitingPayment(order)) return false;
      if (paymentFilter === "ips" && order.paymentMethod !== "ips") return false;
  ...
  }, [orders, search, statusFilter, paymentFilter, cityFilter, noteOnly, dateFrom, dateTo, amountMin, amountMax, sortBy]);
  ...
  const filtersApplied = useMemo(() => {
    ...
    return count;
  }, [search, statusFilter, paymentFilter, cityFilter, noteOnly, dateFrom, dateTo, amountMin, amountMax, sortBy]);

  useEffect(() => {
    setPage(1);
  }, [search, statusFilter, paymentFilter, cityFilter, noteOnly, dateFrom, dateTo, amountMin, amountMax, sortBy, rowsPerPage]);
  ...
  const markPaymentConfirmed = async (order: AdminOrder) => {
    setBusyOrderId(order._id);
    try {
      await confirmOrderPayment({ orderId: order._id, confirmed: true });
      setFeedback({
        type: "success",
        message: `Uplata za narudžbinu ${order.orderNumber} je potvrđena. Sada možete da je pošaljete.`,
      });
    } catch (error: unknown) {
      const message = error instanceof Error && error.message ? error.message : "Potvrda uplate nije uspela.";
  ...
  const copyPaymentReference = async (order: AdminOrder) => {
    const reference = order.paymentReference?.trim();
    if (!reference) return;
    try {
      await copyTextToClipboard(reference);
      setFeedback({ type: "success", message: `Poziv na broj ${reference} je kopiran.` });
    } catch {
      setFeedback({ type: "error", message: "Kopiranje poziva na broj nije uspelo." });
    }
  };
  ...
  const updateStatus = async (order: AdminOrder, nextStatus: OrderStatus) => {
    ...
    if (isAwaitingPayment(order) && (nextStatus === "processed" || nextStatus === "completed")) {
      setFeedback({
        type: "error",
        message: `Narudžbina ${order.orderNumber} je plaćena IPS-om, a uplata još nije potvrđena. Prvo kliknite "Potvrdi uplatu".`,
      });
      return;
    }
      ...
      <section className={`toolbar-card ${styles.filterPanel}`}>
        ...
        <div className={styles.quickStatusRow}>
          ...
          {summary.awaitingPayment > 0 ? (
            <button
              type="button"
              className={`${styles.quickStatusChip} ${styles.awaitingPaymentChip} ${
                paymentFilter === "awaiting" ? styles.activeChip : ""
              }`}
              onClick={() => setPaymentFilter(paymentFilter === "awaiting" ? "all" : "awaiting")}
            >
              Čeka uplatu ({summary.awaitingPayment})
            </button>
          ) : null}
        </div>
        ...
        <div className={styles.filterGrid}>
          ...
          <label className={styles.field}>
            ...
            <select value={paymentFilter} onChange={(event) => setPaymentFilter(event.target.value as PaymentFilter)}>
              ...
              <option value="awaiting">Čeka uplatu (IPS)</option>
              <option value="ips">IPS online</option>
                ...
                {pagedOrders.map((order) => {
                  ...
                  return (
                    <Fragment key={order._id}>
                      <tr className={`${styles.orderRow} ${awaitingPayment ? styles.awaitingPaymentRow : ""}`}>
                        <td>
                            ...
                            {order.paymentMethod === "ips" ? (
                              <span
                                className={`${styles.paymentBadge} ${
                                  awaitingPayment ? styles.paymentBadgeAwaiting : styles.paymentBadgeConfirmed
                                }`}
                              >
                                {awaitingPayment ? "IPS ONLINE — ČEKA UPLATU" : "IPS ONLINE — UPLATA POTVRĐENA"}
                              </span>
                            ) : (
                              <span className={`${styles.paymentBadge} ${styles.paymentBadgeCod}`}>
                                POUZEĆE — naplata {formatRsd(order.totals.totalAmount)}
                              </span>
                            )}
                            {order.freeShipping ? (
                              <span className={`${styles.paymentBadge} ${styles.paymentBadgeShipping}`}>
                                BESPLATNA DOSTAVA
                              </span>
                            ) : null}
                          </div>
                        ...
                        <td>
                          ...
                          <div className={styles.statusCell}>
                            ...
                            >
                              <option value="pending">Novo</option>
                              <option value="processed" disabled={awaitingPayment}>
                                U obradi
                              </option>
                              <option value="completed" disabled={awaitingPayment}>
                                Završeno
                              </option>
                            </select>
                            {awaitingPayment ? (
                              <small className={styles.awaitingPaymentHint}>
                                Slanje je zaključano dok uplata nije potvrđena.
                              </small>
                            ) : null}
                          </div>
                        </td>
                        <td className={styles.actionsCell}>
                          <div className={styles.rowActions}>
                            {awaitingPayment ? (
                              <button
                                type="button"
                                className={`primary-btn ${styles.confirmPaymentButton}`}
                                onClick={() => void markPaymentConfirmed(order)}
                                disabled={isBusy}
                                aria-label={`Potvrdi uplatu za narudžbinu ${order.orderNumber}`}
                              >
                                Potvrdi uplatu
                              </button>
                            ) : null}
                            {order.paymentMethod === "ips" && order.paymentReference ? (
                              <button
                                type="button"
                                className={`ghost-btn ${styles.copyButton}`}
                                onClick={() => void copyPaymentReference(order)}
                                disabled={isBusy}
                                aria-label={`Kopiraj poziv na broj za narudžbinu ${order.orderNumber}`}
                              >
                                Kopiraj poziv na broj
                              </button>
                            ) : null}
                        ...
                        <tr className={styles.detailRow}>
                          <td colSpan={7}>
                            <div className={styles.detailGrid}>
                              ...
                              <article className={styles.detailCard}>
                                    ...
                                    <li key={`${order._id}-${item.productId}-${index}`}>
                                      ...
                                      <div>
                                        <strong>
                                          {item.title}
                                          {item.bonusApplied ? (
                                            <span className={styles.bonusTag}>BONUS −15%</span>
                                          ) : null}
                                        </strong>
                                        <span>
                                          {item.quantity} x {formatRsd(item.payableUnitPrice ?? item.finalUnitPrice)}
                                ...
                                <div className={styles.detailTotals}>
                                  <p>
                                    ...
                                    <strong>{formatRsd(order.totals.goodsTotal ?? order.totals.totalAmount)}</strong>
                                  </p>
                                  {(order.totals.bonusSavings ?? 0) > 0 ? (
                                    <p>
                                      <span>Bonus popust −15%</span>
                                      <strong>−{formatRsd(order.totals.bonusSavings ?? 0)}</strong>
                                    </p>
                                  ) : null}
                                  <p>
                                    <span>Dostava</span>
                                    <strong>
                                      {order.freeShipping ? "Besplatna (plaća studio)" : "Plaća kupac kuriru"}
                                    </strong>
                                  </p>
                                  <p className={styles.detailTotalsFinal}>
                              ...
                              <article
                                className={`${styles.detailCard} ${awaitingPayment ? styles.detailCardAlert : ""}`}
                              >
                                <h4>Plaćanje</h4>
                                {order.paymentMethod === "ips" ? (
                                  <>
                                    <p>
                                      <strong>
                                        {awaitingPayment
                                          ? "IPS online — uplata NIJE potvrđena"
                                          : "IPS online — uplata potvrđena"}
                                      </strong>
                                    </p>
                                    {awaitingPayment ? (
                                      <p className={styles.noteBadge}>
                                        Ne slati kao pouzeće. Proverite priliv na računu, pa kliknite „Potvrdi uplatu”.
                                        Uplate stigle van radnog vremena banke mogu da legnu tek narednog radnog dana.
                                      </p>
                                    ) : (
                                      <p>Potvrđeno: {formatDateTime(order.paymentConfirmedAt ?? 0)}</p>
                                    )}
                                    {order.paymentReference ? <p>Poziv na broj: {order.paymentReference}</p> : null}
                                    {order.paymentPurpose ? <p>Svrha uplate: {order.paymentPurpose}</p> : null}
                                  </>
                                ) : (
                                  <>
                                    <p>
                                    ...
                                    <p>Naplatiti {formatRsd(order.totals.totalAmount)} pri preuzimanju.</p>
                                  </>
                                )}
                              </article>
                            </div>
