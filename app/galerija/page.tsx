...
export default function GalleryPage() {
  ...
  return (
    <section className="page-grid orbit-page gallery-orbit">
      <article className="gallery-hero-section orbit-reveal">
        <div className="gallery-hero-hud" aria-hidden>
          <span>Transformacije</span>
          <strong>{isAdmin ? "Galerija radova | Studio Lady Gaga" : "Galerija inspiracija | Studio Lady Gaga"}</strong>
        </div>

        <p className="gallery-hero-eyebrow">{t.nav.gallery}</p>

        <h1 className="gallery-hero-title">
          {isAdmin
            ? "Slike i snimci koji prikazuju realne rezultate iz studija."
            : <>Galerija transformacija iz studija <em>Lady Gaga.</em></>}
        </h1>

        <p className="gallery-hero-lead">
          {isAdmin
            ? "Klikni na karticu za uvecanje i listanje. Admin može da doda fajlove prevlacenjem i pustanjem bilo gde."
            : "Pogledaj poslednje transformacije i otvori kadar preko celog ekrana za detaljan prikaz."}
        </p>

        {!isAdmin ? (
          <div className="gallery-hero-actions">
            <Link href="/kontakt" className="gallery-hero-btn-primary">
              Kontakt / upit
            </Link>
        ...
        <div className="gallery-hero-stats">
          {isAdmin ? (
            ...
            <>
              ...
              <GalleryStatCard value="Uživo" label="sveže transformacije" />
...
function GalleryStatCard({ value, label }: { value: string; label: string }) {
  return (
    <article className="gallery-stat-card">
      <span className="gallery-stat-card-watermark" aria-hidden>✦</span>
      <strong className="gallery-stat-value">{value}</strong>
      <span className="gallery-stat-label">{label}</span>
    </article>
  );
}
