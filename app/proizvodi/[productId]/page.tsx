...
function getCategoryNames(product: Pick<Product, "categoryId" | "categoryIds">, categories: Category[]) {
...
export default function ProductDetailsPage() {
  ...
  const relatedProducts = useMemo(() => {
    if (!product) return [];
    const productCategoryIds = new Set(getProductCategoryIds(product));
    return products
      .filter((item) => item._id !== product._id && getProductCategoryIds(item).some((categoryId) => productCategoryIds.has(categoryId)))
      .slice(0, 4);
  }, [products, product]);
  ...
  const categoryName = categoryNames.length > 0 ? categoryNames.join(", ") : "Bez kategorije";
  const categoryMetaLabel = categoryNames.length > 1 ? "Kategorije" : "Kategorija";
    ...
    <section className="page-grid orbit-page product-detail-page">
      ...
      <article className="product-detail-shell orbit-reveal">
        ...
        <div className="product-detail-info-column">
          ...
          <CartRewardHint showWhenEmpty className="product-detail-reward-hint" />
      ...
      {relatedProducts.length > 0 ? (
        <section className="orbit-panel product-detail-related orbit-reveal">
          ...
          <div className="product-detail-related-head">
            ...
            <span>Iz povezanih kategorija</span>
          </div>
