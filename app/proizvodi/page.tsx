...
import { SHORT_NAME_MAX_LENGTH, productShortName } from "@/lib/ips-purpose";
...
function getProductCategoryIds(product: Pick<Product, "categoryId" | "categoryIds">) {
  return product.categoryIds && product.categoryIds.length > 0 ? product.categoryIds : product.categoryId ? [product.categoryId] : [];
}

function formatCategoryNames(categoryIds: string[], categoriesById: Map<string, string>) {
  const names = categoryIds.map((categoryId) => categoriesById.get(categoryId)).filter((name): name is string => Boolean(name));
...
export default function ProductsPage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    const items = products.filter((product) => {
      const inCategory = activeCategory === "all" || getProductCategoryIds(product).includes(activeCategory);
  ...
  const visibleProductTotal = Math.min(visibleProductCount, filteredProducts.length);
  const hasMoreProducts = visibleProductTotal < filteredProducts.length;

  const loadMoreProducts = useCallback(() => {
    setVisibleProductCount((current) => Math.min(current + PRODUCT_BATCH_SIZE, filteredProducts.length));
  ...
  const toggleFormCategory = (categoryId: string, checked: boolean) => {
    setForm((current) => {
      const currentIds = new Set(current.categoryIds);
      if (checked) currentIds.add(categoryId);
      else currentIds.delete(categoryId);
      return { ...current, categoryIds: [...currentIds] };
    });
  };
      ...
      {rawProducts === undefined || rawCategories === undefined ? (
        ...
        <>
          ...
          {visibleProducts.map((product, productIndex) => {
            ...
            const categoryName = formatCategoryNames(getProductCategoryIds(product), categoriesById);
          ...
          <div className="boutique-load-more" ref={loadMoreRef} aria-live="polite">
            <p className="boutique-load-more-count">
              Prikazano {visibleProductTotal} od {filteredProducts.length} proizvoda
            </p>
            {hasMoreProducts ? (
              <button className="ghost-btn boutique-load-more-btn" type="button" onClick={loadMoreProducts}>
                Prikazi jos
              </button>
            ) : (
              <p className="boutique-load-more-end">Prikazani su svi proizvodi.</p>
            )}
          </div>
        </>
      )}
      ...
      {showProductModal ? (
        <Modal onClose={requestCloseProductModal}>
          ...
          <form className="modal-form admin-product-form" onSubmit={onSaveProduct}>
            ...
            <label className="admin-shortname-field">
              <span>Skraćeni naziv za uplatnicu (IPS QR)</span>
              <input
                maxLength={SHORT_NAME_MAX_LENGTH}
                placeholder={productShortName(form.title) || "npr. Sampon Kerat"}
                value={form.shortName}
                onChange={(event) => setForm((value) => ({ ...value, shortName: event.target.value }))}
              />
              <small>
                Svrha uplate na IPS QR kodu sme da ima najviše 35 karaktera. Ostavite prazno i naziv se generiše
                automatski: <strong>{productShortName(form.title) || "Proizvod"}</strong>
              </small>
            </label>
            ...
            <fieldset className="admin-category-checkbox-group">
              <legend>Kategorije</legend>
              {categories.length === 0 ? (
                <p className="admin-category-checkbox-empty">Prvo dodajte kategoriju.</p>
              ) : (
                <div className="admin-category-checkbox-list">
                  {categories.map((category) => {
                    const checked = form.categoryIds.includes(category._id);
                    return (
                      <label key={category._id} className={`admin-category-checkbox${checked ? " is-on" : ""}`}>
                        <input
                          type="checkbox"
                          checked={checked}
                          onChange={(event) => toggleFormCategory(category._id, event.target.checked)}
                        />
