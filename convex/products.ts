...
import { SHORT_NAME_MAX_LENGTH, toAscii } from "../lib/ips-purpose";

type ProductDoc = Doc<"products">;

/** Skraćeni naziv ide u svrhu uplate na IPS QR kodu — mora ASCII i kratak. */
function normalizeShortName(value: string | undefined) {
  const normalized = toAscii(value ?? "").slice(0, SHORT_NAME_MAX_LENGTH).trim();
...
function normalizeCategoryIds(categoryIds: Id<"categories">[] | undefined, categoryId: Id<"categories"> | undefined) {
  const ids = categoryIds && categoryIds.length > 0 ? categoryIds : categoryId ? [categoryId] : [];
  return [...new Set(ids)];
}

function resolveProductCategoryIds(product: Pick<ProductDoc, "categoryId" | "categoryIds">) {
  return normalizeCategoryIds(product.categoryIds, product.categoryId);
}

function formatProductCategoryNames(product: Pick<ProductDoc, "categoryId" | "categoryIds">, categoryById: Map<string, string>) {
...
async function normalizeProductForClient(ctx: QueryCtx, product: ProductDoc) {
  ...
  const categoryIds = resolveProductCategoryIds(product);
...
export const getSeoMeta = query({
  ...
  handler: async (ctx, args) => {
    ...
    const categoryNames = resolveProductCategoryIds(product)
      .map((categoryId) => categoryById.get(categoryId as string))
      .filter((name): name is string => Boolean(name));

    const image = await resolvePrimaryImage(ctx, product);
    ...
    return {
      ...
      price: product.price,
      finalPrice: resolveFinalUnitPrice(product.price, discount),
      discount,
      stock: product.stock,
      categoryNames,
      image,
...
export const seoSitemapProducts = query({
  args: {},
  handler: async (ctx) => {
    const products = await ctx.db.query("products").collect();
    return products.map((product) => ({ _id: product._id }));
  },
});
...
export const homeSnapshot = query({
  ...
  handler: async (ctx) => {
    ...
    for (const product of products) {
      ...
      const categoryKeys = productCategoryIds.length > 0 ? productCategoryIds : ["__uncategorized__"];
    ...
    const serializeProductForHome = async (product: ProductDoc) => ({
      ...
      categoryName: formatProductCategoryNames(product, categoryById),
...
export const deleteCategory = mutation({
  ...
  handler: async (ctx, args) => {
    const products = await ctx.db.query("products").collect();
    const linkedProducts = products.filter((product) => resolveProductCategoryIds(product).includes(args.categoryId));
...
export const upsertProduct = mutation({
  ...
  handler: async (ctx, args) => {
    if (args.productId) {
      ...
      const categoryIds =
        args.categoryIds !== undefined || args.categoryId !== undefined
          ? normalizeCategoryIds(args.categoryIds, args.categoryId)
          : resolveProductCategoryIds(existing);
      ...
      const payload = {
        ...
        shortName: normalizeShortName(args.shortName),
    ...
    const categoryIds = normalizeCategoryIds(args.categoryIds, args.categoryId);
    if (normalizedImages.length === 0 && storageImageIds.length === 0) {
      throw new Error("At least one image is required.");
    }
