...
import { FREE_SHIPPING_THRESHOLD } from "@/lib/cart-rewards";
...
export function Navbar() {
  ...
  const rewardProgress = Math.min(1, rewards.goodsTotal / FREE_SHIPPING_THRESHOLD);
      ...
      {itemCount > 0 ? (
        <>
          <svg className="cart-nav-ring" viewBox="0 0 44 44" aria-hidden focusable="false">
            ...
            <circle
              className={`cart-nav-ring-fill ${rewards.freeShipping ? "is-complete" : ""}`}
              cx="22"
              cy="22"
              r="20"
