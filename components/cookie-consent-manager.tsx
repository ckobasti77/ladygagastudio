...
export function CookieConsentManager() {
  ...
  const consentModeLabel = useMemo(() => {
    ...
    if (savedConsent.analytics && savedConsent.marketing) return "Puni režim";
    if (!savedConsent.analytics && !savedConsent.marketing) return "Samo neophodni";
    return "Prilagođeni režim";
  }, [savedConsent]);
  ...
  return (
    <>
      <section className={`cookie-starport ${showBanner ? "visible" : ""}`} aria-live="polite" aria-label="Podešavanja kolačića">
        <div className="cookie-starport-shell">
          <div className="cookie-starport-copy">
            ...
            <div className="cookie-preset-grid" aria-label="Brzi režimi saglasnosti">
              <button
                type="button"
        ...
        >
          <section className="cookie-lab-panel" role="dialog" aria-modal="true" aria-labelledby="cookie-lab-title">
            ...
            <div className="cookie-preset-grid modal" aria-label="Brzi izbor profila">
              ...
              >
                <span>Balansirani</span>
                <small>Analitika uključena</small>
              </button>
              <button
                type="button"
                className={`cookie-preset-btn ${isSameDraft(draft, FULL_DRAFT) ? "active" : ""}`}
      ...
      {savedConsent && !showBanner && !showPreferences ? (
        <>
          <button type="button" className="cookie-orbit-launcher" onClick={openPreferencesPanel} aria-label="Otvori panel privatnosti">
            <Cookie aria-hidden="true" />
            <span className="cookie-orbit-label">Privatnost</span>
          </button>
